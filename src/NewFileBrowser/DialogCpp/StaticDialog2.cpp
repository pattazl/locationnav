...
void StaticDialog2::display(bool toShow) const 
{
	if (toShow) {
...
HGLOBAL StaticDialog2::makeRTLResource(int dialogID, DLGTEMPLATE **ppMyDlgTemplate)
{
...
void StaticDialog2::create(int dialogID, bool isRTL, bool isModeles)
{
	if (isRTL)
	{
	...
	if (!_hSelf)
	{
		//systemMessage(_T("StaticDialog2"));
		throw int(666);
	}
...
BOOL CALLBACK StaticDialog2::dlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message) 
	{
		case WM_INITDIALOG :
		{
			StaticDialog2 *pStaticDlg = (StaticDialog2 *)(lParam);
		...
		default :
		{
			StaticDialog2 *pStaticDlg = reinterpret_cast<StaticDialog2 *>(::GetWindowLong(hwnd, GWL_USERDATA));
			if (!pStaticDlg)
				return FALSE;
...
void StaticDialog2::alignWith(HWND handle, HWND handle2Align, PosAlign pos, POINT & point)
{
    RECT rc, rc2;
